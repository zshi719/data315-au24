...
p <- ggplot(combined_changes, aes(
  ...
  scale_color_manual(values = c("Added" = pal[1], "Removed" = pal[2])) + # Use `pal` for colors
  labs(
    title = "S&P 500 Company Changes: {frame_time}",
    subtitle = paste("Added =", pal[1], ", Removed =", pal[2]), # Display color names dynamically
    x = "Longitude",
    y = "Latitude",
    color = "Change Type"
  ) +
...
# Add interactive labels with gganimate and save the animation
animated_plot <- animate(p, nframes = 100, fps = 10, width = 800, height = 600)
# save mp4 file
anim_save("sp500_changes.mp4", animation = animated_plot)
```
